...
export class ManageCouponsComponent implements OnInit {
  ...
  // FIX: Create local signals to hold asynchronous data
  private _employees = signal<Employee[]>([]);
  private _contractors = signal<Contractor[]>([]);
  private _coupons = signal<Coupon[]>([]);

  // --- Employee Tab State & Logic ---
  employees = computed(() => this._employees().filter(e => e.role === 'employee' && e.status === 'active'));
  ...
  generateCouponsForm = new FormGroup({
    couponType: new FormControl<Coupon['couponType'] | null>(null, [Validators.required])
  });
  ...
  generateContractorCouponsForm = new FormGroup({
    couponType: new FormControl<Coupon['couponType'] | null>(null, [Validators.required]),
    quantity: new FormControl(1, [Validators.required, Validators.min(1), Validators.max(500)])
  });
  ...
  employeesMap = computed(() => {
    ...
    this._employees().forEach(emp => map.set(emp.id, emp));
    return map;
  });
  ...
  async loadData() {
    try {
      ...
      this._contractors.set(await this.dataService.getContractors());
      this._coupons.set(await this.dataService.getCoupons());
    } catch (error) {
      console.error("Failed to load coupon management data:", error);
    }
  }
  ...
  employeeCouponStats = computed(() => {
    ...
    for (const employee of this.employees()) {
        statsMap.set(employee.id, { totalIssued: 0, totalRedeemed: 0, breakdown: {} });
    }
  ...
  employeeDateStats = computed(() => {
    const statsMap = new Map<number, { lastIssued: string | null, lastRedeemed: string | null }>();
    const allCoupons = this._coupons();
    for (const employee of this.employees()) {
    ...
    for (const coupon of allCoupons) {
        if (coupon.employeeId && statsMap.has(coupon.employeeId)) {
            // FIX: Use coupon.employeeId to get stats, not the out-of-scope 'employee' variable.
            const currentStats = statsMap.get(coupon.employeeId)!;
            if (!currentStats.lastIssued || new Date(coupon.dateIssued) > new Date(currentStats.lastIssued)) {
                currentStats.lastIssued = coupon.dateIssued;
            }
            if (coupon.status === 'redeemed' && coupon.redeemDate) {
                if (!currentStats.lastRedeemed || new Date(coupon.redeemDate) > new Date(currentStats.lastRedeemed)) {
  ...
  employeeUnredeemedCoupons = computed(() => {
    ...
    if (!employee) return [];
    // Depend on the signal directly for reactivity
    const allCoupons = this._coupons();
    return allCoupons
        .filter(c => c.employeeId === employee.id && c.status === 'issued')
        .sort((a, b) => new Date(a.dateIssued).getTime() - new Date(b.dateIssued).getTime());
  });
  ...
  lastBatchInfo = computed(() => {
    ...
    const lastBatchCoupons = coupons.filter(c => c.dateIssued === mostRecentDate);
  ...
  contractorCouponStats = computed(() => {
    const statsMap = new Map<number, { breakdown: Partial<Record<Coupon['couponType'], number>> }>();
    const allCoupons = this._coupons();
    for (const contractor of this.contractors()) {
        statsMap.set(contractor.id, { breakdown: {} });
    }
    for (const coupon of allCoupons) {
        if (coupon.contractorId && !coupon.employeeId && statsMap.has(coupon.contractorId)) {
  ...
  filteredGuestCoupons = computed(() => {
    ...
    return this.guestCoupons().filter(coupon => {
        const employeeName = coupon.sharedByEmployeeId ? (empMap.get(coupon.sharedByEmployeeId)?.name || 'unknown') : 'unknown';

        return coupon.couponType.toLowerCase().includes(term) ||
  ...
  onDocumentClick(event: MouseEvent) {
    const exportMenu = this.elementRef.nativeElement.querySelector('.export-menu-container');
    if (this.isExportMenuOpen() && exportMenu && !exportMenu.contains(event.target as Node)) {
  ...
  openGenerateContractorCouponsModal(contractor: Contractor) {
    ...
    this.generateContractorCouponsForm.reset({ couponType: null, quantity: 1 });
  ...
  async handleGenerateCoupons() {
    ...
    if (this.generateCouponsForm.valid && this.selectedEmployee()) {
        ...
        const result = await this.dataService.generateCouponsForEmployee(this.selectedEmployee()!.id, couponType!);
        if (result.success) {
  ...
  async handleGenerateContractorCoupons() {
    this.generateContractorCouponError.set(null);
    if (this.generateContractorCouponsForm.valid && this.selectedContractor()) {
      ...
      const result = await this.dataService.generateCouponsForContractor(this.selectedContractor()!.id, couponType!, quantity!);
       if (result.success) {
          this.statusMessage.set({ type: 'success', text: result.message });
          this.closeModals();
          this._coupons.set(await this.dataService.getCoupons());
  ...
  async handleRemoveCoupon(couponId: string) {
    if (confirm('Are you sure you want to permanently remove this coupon? This action cannot be undone.')) {
        const result = await this.dataService.removeCoupon(couponId);
        if (result.success) {
            this.statusMessage.set({ type: 'success', text: result.message });
            this._coupons.set(await this.dataService.getCoupons());
        } else {
  ...
  async handleRemoveLastBatch() {
    const employee = this.selectedEmployeeForCouponMgmt();
    if (!employee) return;
    
    const result = await this.dataService.removeLastCouponBatch(employee.id);
    this.isRemoveLastBatchModalOpen.set(false);
  ...
  getCouponTypeClass(couponType: Coupon['couponType']): string {
    switch (couponType) {
      ...
      case 'Lunch/Dinner': return 'bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300';
  ...
  exportCouponsCsv() {
    ...
    const headers = ['Name', 'Employee ID', 'Department', 'Role', ...breakdownHeaders, 'Total (I/R)', 'Last Issued On', 'Last Redeemed On'];
    ...
    employeesToExport.forEach(emp => {
        ...
        const dateStats = dateStatsMap.get(emp.id) || { lastIssued: null, lastRedeemed: null };
        ...
        const totalValue = stats ? `"${stats.totalIssued}/${stats.totalRedeemed}"` : '"0/0"';
        const row = [`"${emp.name}"`, emp.employeeId, emp.department || 'N/A', emp.role, ...breakdownValues, totalValue, this.formatDateTime(dateStats.lastIssued), this.formatDateTime(dateStats.lastRedeemed)].join(',');
        csvContent += row + '\n';
    });
    this.downloadFile(csvContent, 'hyva_pune_canteen_coupon_report.csv', 'text/csv;charset=utf-8;');
  }
  ...
  exportCouponsPdf() {
    ...
    const head = [['Name', 'ID', 'Dept', ...breakdownHeaders, 'Total', 'Last Issued', 'Last Redeemed']];
    const body = employeesToExport.map(emp => {
        ...
        const totalValue = stats ? `${stats.totalIssued}/${stats.totalRedeemed}` : '0/0';
        return [emp.name, emp.employeeId, emp.department || 'N/A', ...breakdownValues, totalValue, this.formatDateTime(dateStats.lastIssued), this.formatDateTime(dateStats.lastRedeemed)];
    });
    doc.setFontSize(18);
    ...
    jspdf.autoTable(doc, { startY: 35, head: head, body: body, theme: 'striped', headStyles: { fillColor: [75, 85, 99] }, styles: { fontSize: 7, cellPadding: 1.5 }, });
