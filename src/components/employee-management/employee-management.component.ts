...
export class EmployeeManagementComponent implements OnInit {
  ...
  // FIX: Use a local signal for employees
  private _employees = signal<Employee[]>([]);
  employees = computed(() => this._employees().filter(e => e.employeeId !== 'admin01'));
  ...
  // FIX: Fetch contractors asynchronously
  private _contractors = signal<Contractor[]>([]);
  contractors = computed(() => this._contractors().map(c => c.businessName));
  ...
  async handleUpdateEmployee() {
    if (this.editEmployeeForm.valid) {
      ...
      const originalEmployee = this._employees().find(e => e.id === formValue.id);
      ...
      if (originalEmployee) {
        ...
        await this.dataService.updateEmployee(updatedEmployee);
  ...
  async confirmDelete() {
    ...
    if (employeeToDelete) {
        await this.dataService.deleteEmployee(employeeToDelete.id);
  ...
  async confirmToggleStatus() {
    ...
    if (employee) {
      await this.dataService.toggleEmployeeStatus(employee.id);
