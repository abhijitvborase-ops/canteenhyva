...
export class ManageContractorsComponent implements OnInit {
  ...
  // FIX: Use a local signal to store contractors fetched asynchronously.
  ...
  async handleAddContractor() {
    if (this.contractorForm.valid) {
      const { name, businessName, contractorId, password } = this.contractorForm.value;
      await this.dataService.addContractor({ name, businessName, contractorId, password });
  ...
  async handleUpdateContractor() {
    if (this.contractorForm.valid) {
      ...
      if (originalContractor) {
        ...
        await this.dataService.updateContractor(updatedContractor);
  ...
  async confirmDelete() {
    ...
    if (contractorToDelete) {
        await this.dataService.deleteContractor(contractorToDelete.id);
